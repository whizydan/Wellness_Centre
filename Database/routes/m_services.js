...
/* GET bookings for a specific masseur with status = 1 */
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request()
            .input('id', sql.Int, id)
            .query(`SELECT * FROM bookings INNER JOIN services ON bookings.service = services.id WHERE masseurId = @id AND status = 1`);
